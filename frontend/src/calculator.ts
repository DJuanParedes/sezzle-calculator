export const operations = [
  {
    ...
    label: "Add",
    symbol: "+",
    description: "Bring two numbers together.",
  },
  {
    id: "subtract",
    label: "Subtract",
  ...
  },
  {
    ...
    label: "Power",
    symbol: "^",
    description: "Raise the first number to a power.",
  },
  {
    id: "sqrt",
    label: "Square root",
    symbol: "√",
    description: "Find the non-negative square root.",
  },
  {
    id: "percentage",
    label: "Percentage",
    symbol: "%",
    description: "Find a percentage of a number: 15% of 200 is 30.",
  },
] as const;
...
export function parseOperands(
  ...
  const values = inputs.map((input, index) => {
    ...
    if (!decimal.test(text) || !Number.isFinite(Number(text))) {
      throw new Error(
        `Enter a finite number for ${index === 0 ? "the first value" : "the second value"}.`,
      );
    }
    return Number(text);
  });
  if (operation === "divide" && values[1] === 0)
    throw new Error("Cannot divide by zero.");
  if (operation === "sqrt" && values[0] < 0)
