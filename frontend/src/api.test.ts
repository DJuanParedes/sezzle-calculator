...
describe('API client', () => {
 it('posts numeric operands and forwards the cancellation signal', async () => {
  const fetch = vi.fn().mockResolvedValue({ ok: true, json: async () => ({ result: 5 }) })
 ...
 it('surfaces a structured server error', async () => {
  vi.stubGlobal('fetch', vi.fn().mockResolvedValue({ ok: false, json: async () => ({ error: { message: 'Cannot divide by zero.' } }) }))
  await expect(calculate(input)).rejects.toThrow('Cannot divide by zero.')
 })
 it.each([null, {}, { error: { message: 42 } }])('handles unknown error shape %j', async data => {
 ...
 it.each([null, {}, { result: '5' }, { result: Infinity }, { result: NaN }])('rejects invalid success %j', async data => {
  vi.stubGlobal('fetch', vi.fn().mockResolvedValue({ ok: true, json: async () => data }))
  await expect(calculate(input)).rejects.toThrow('invalid result')
 })
 it('handles a non-JSON response', async () => {
  vi.stubGlobal('fetch', vi.fn().mockResolvedValue({ ok: false, json: async () => { throw new Error('JSON') } }))
