...
describe('calculator UI', () => {
 ...
 it('submits via Enter and uses the server result', async () => {
  ...
  await waitFor(() => expect(screen.getByRole('status')).toHaveTextContent('32'))
  expect(api).toHaveBeenCalledWith({ operation: 'add', operands: [24, 8] }, expect.any(AbortSignal))
  expect(screen.getByRole('region', { name: 'Recent calculations' })).toHaveTextContent('24 + 8')
 })
 it('supports all operation buttons and unary square root', async () => {
  ...
  expect(api).toHaveBeenCalledWith({ operation: 'sqrt', operands: [9] }, expect.any(AbortSignal))
 ...
 it('labels and submits percentage unambiguously', async () => {
  ...
  await user.type(screen.getByLabelText('Percentage', { selector: 'input' }), '15')
  await user.type(screen.getByLabelText('Of this number'), '200')
  await user.click(screen.getByRole('button', { name: 'Calculate' }))
  await waitFor(() => expect(screen.getByRole('status')).toHaveTextContent('15% of 200'))
  expect(api).toHaveBeenCalledWith({ operation: 'percentage', operands: [15, 200] }, expect.any(AbortSignal))
 })
 ...
 it('disables editing and avoids duplicate requests while pending', async () => {
  ...
  fireEvent.submit(screen.getByRole('button', { name: 'Calculating…' }).closest('form')!)
 ...
 it('reports API errors and permits retry', async () => {
  api.mockRejectedValueOnce(new Error('The result is not a finite real number.')).mockResolvedValueOnce(32)
  render(<App />)
  const user = await fill()
  await user.click(screen.getByRole('button', { name: 'Calculate' }))
  expect(await screen.findByRole('alert')).toHaveTextContent('finite real number')
  await user.click(screen.getByRole('button', { name: 'Calculate' }))
  await waitFor(() => expect(screen.getByRole('status')).toHaveTextContent('32'))
 ...
 it('keeps only five calculations and can clear history', async () => {
  ...
  for (let i = 0; i < 6; i++) {
   await user.click(screen.getByRole('button', { name: 'Calculate' }))
   await waitFor(() => expect(screen.getByRole('button', { name: 'Calculate' })).toBeEnabled())
  }
 ...
 it('times out a stalled request and aborts it', async () => {
  vi.useFakeTimers()
  api.mockImplementation((_input, signal) => new Promise((_resolve, reject) => {
   signal?.addEventListener('abort', () => reject(new DOMException('Aborted', 'AbortError')))
  }))
  render(<App />)
  fireEvent.change(screen.getByLabelText('First value'), { target: { value: '1' } })
  fireEvent.change(screen.getByLabelText('Second value'), { target: { value: '2' } })
